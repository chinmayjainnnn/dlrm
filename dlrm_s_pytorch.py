...
class DLRM_Net(nn.Module):
    ...
    def create_emb(self, m, ln, weighted_pooling=None):
        emb_l = nn.ModuleList()
        # emb_l: This list stores the embedding tables as nn.ModuleList, allowing the network to handle multiple categorical feature embeddings.
        v_W_l = []
        # v_W_l: This list holds the weights for each feature if weighted_pooling is used, initialized as None for features without weighting.
        for i in range(0, ln.size):
            if ext_dist.my_size > 1:
                if i not in self.local_emb_indices:
                    continue
            n = ln[i]
            # print(f'embedding dimensions{n,m}')
            # construct embedding operator
            if self.use_hashed_embedding:
                EE=HashEmbedding(
                num_embeddings=n,
                embedding_dim=m,
    ...
    def __init__(
        ...
        if (
            ...
            #create variables for Hashembeddings
            self.use_hashed_embedding=use_hashed_embedding
            if self.use_hashed_embedding:
                print("using hash embeddings")
    ...
    def apply_emb(self, lS_o, lS_i, emb_l, v_W_l):
        ...
        # lS_o: A list of offset tensors for each embedding table. Each entry contains offset values marking the start of each sequence of indices in the batch.
        # lS_i: A list of index tensors corresponding to the indices for each embedding table. These indices specify which rows to fetch from each embedding table.
        # emb_l: A list of embedding tables created previously by create_emb
        # v_W_l: A list of weight vectors, used when weighted_pooling is applied. Each weight vector corresponds to an embedding table.
        # ly: This list stores the output embeddings for each feature after lookup and pooling.
        ...
        for k, sparse_index_group_batch in enumerate(lS_i):
            ...
            if self.quantize_emb:
                ...
                E = emb_l[k]
                if isinstance(E,HashEmbedding):
                    
                    V=E(sparse_index_group_batch,sparse_offsets=sparse_offset_group_batch,per_sample_weights=per_sample_weights)
                    # print("shape ofV",V.shape)
    ...
    def sequential_forward(self, dense_x, lS_o, lS_i):
        ...
        ly = self.apply_emb(lS_o, lS_i, self.emb_l, self.v_W_l)
        # print(f'after_apply_emb{ly[0].shape}')
        # for y in ly:
...
# ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#                       code to load the mlp layer weights to dlrm instance and saved it in the end
# ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////



    checkpoint=torch.load("/Data/chinmay/dlrm/criteo_pretrain_model/tb0875_10M.pt")
    pretrained_state_dict =checkpoint['state_dict']
    
    # embedding_tables={}sto
    # for key,value in state_dict.items():
    #     if 'emb_l' in key:
    #         embedding_tables[key]=value

    
    # for name, weights in embedding_tables.items():
    #     np.save(f"{name}.npy", weights)

    # print("Embedding tables saved as .npy files")
    ln_bot=[13, 512, 256, 64, 16]
    ln_top=[415, 512, 512, 256, 1]
    ...
    new_state_dict = dlrm1.state_dict()
    
    for name, param in pretrained_state_dict.items():
        if "bot_l" in name or "top_l" in name:  # Only load dense layer weights
            if name in new_state_dict and new_state_dict[name].shape == param.shape:
                new_state_dict[name].copy_(param)
            else:
                print(f"Skipping layer {name} due to mismatch.")
        else:
            print("sparse skipped",name)

    # Update model's state dict with the copied weights
    dlrm1.load_state_dict(new_state_dict,strict=False)

    # Save the updated model if needed
    torch.save(dlrm1.state_dict(), 'new_dlrm_with_copied_weights.pt')
    print("][[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]")

# ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#                                   code ended for loading pretrained mlp layer weights
# ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

# ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#                                   code to load the weights of the importance parameters and shared embeddings
# ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    run()