...
class HashEmbedding(nn.Module):
    ...
    def __init__(self, num_embeddings, embedding_dim, num_buckets=None, num_hashes=4, train_sharedEmbed=True,
                 train_weight=True, append_weight=False, aggregation_mode='sum', mask_zero=False, seed=None, oldAlgorithm=False):
        super(HashEmbedding, self).__init__()

        self.num_embeddings = num_embeddings
        self.embedding_dim = embedding_dim
        self.num_hashes = num_hashes
        defaultNBuckets = max(10,(num_embeddings * self.num_hashes) // (self.embedding_dim))
    ...
    def forward(self, input, sparse_offsets=None, per_sample_weights=None):
        ...
        # print("input",input)
        # print("importance",idx_importance_weights)
        # print(f'{self.num_buckets =}')
        # print("offsets",sparse_offsets)
        # max_idx = idx_shared_embeddings.max().item()
        # assert max_idx <= self.num_buckets, f"Hash index {max_idx} exceeds num_buckets {self.num_buckets}"
        shared_embedding = torch.stack([self.shared_embeddings(idx_shared_embeddings[:,  iHash])
                                        for iHash in range(self.num_hashes)], dim=-1)
        # print("ss",shared_embedding.shape)
        importance_weight = self.importance_weights(idx_importance_weights)
        importance_weight = importance_weight.unsqueeze(-2)
        # print("importance matrix",importance_weight.shape)
        word_embedding = self.aggregate(importance_weight * shared_embedding)
        # print("first",word_embedding)
        # print("we after aggregating",word_embedding)
        # print("we shape",word_embedding.shape)
        # print("we",word_embedding)
        if sparse_offsets is not None:
            pooled_embedding=[]
            for i in range(len(sparse_offsets)-1):
                start,end =sparse_offsets[i].item(), sparse_offsets[i+1].item()
                # print(start.item())
                # print(end.item())
                pooled_embedding.append(torch.sum(word_embedding[start:end], dim=0))
            lastoff=sparse_offsets[-1].item()
            
            pooled_embedding.append(torch.sum(word_embedding[lastoff:],dim=0) )
            # lambda x: torch.sum(x, dim=-1)
            # print("pe",pooled_embedding)
            word_embedding=torch.stack(pooled_embedding)
            
        if self.append_weight:
            # concateates the vector with the weights
            word_embedding = torch.cat([word_embedding, importance_weight.squeeze(-2)], dim=-1)
        # print("last we",word_embedding)
